...
- [ ] 新增 Refresh Session 表，至少包含用户、Token 哈希、过期、撤销、创建和最后使用时间。
- [ ] 增加按用户和 Token 哈希查询的索引。
- [ ] Access Token 使用短有效期，具体秒数通过 `expires_in` 返回。
- [ ] 增加刷新、重放、撤销和多用户隔离测试。
...
- [ ] `question_sets` 增加 `updated_at`。
- [ ] `question_sets` 增加可空 `source_question_set_id` 自引用外键。
- [ ] 实现列表、详情、完整替换更新、关联检查删除和重新生成。
- [ ] 增加题号、事务回滚、历史面试快照和多用户隔离测试。
...
- [ ] 新接口上线期间保留旧接口，标记 deprecated。
- [ ] 旧接口内部复用“保存当前题回答”的新业务逻辑。
- [ ] 前端切换到指定题接口并完成一次发布后，再安排删除旧接口。
- [ ] 删除前必须确认无调用方和自动化测试依赖。
...
- [ ] 响应补 `status`、问题、原回答、创建和更新时间。
- [ ] 保持现有一场面试唯一一份报告约束。
- [ ] 增加并发生成复用、未回答题和质量门禁测试。
- [ ] 第一阶段不开发服务端 PDF；由浏览器打印导出。
...
- [ ] `async_tasks` 增加可空 `retry_of_task_id` 自引用外键。
- [ ] 设计结构化、安全的 `error_code` 与用户错误摘要字段。
- [ ] 实现列表、筛选、分页和重试。
- [ ] Worker 对失败、成功和完成时间进行一致更新。
- [ ] 增加重复重试和跨用户访问测试。
...
- [ ] 每项变更提供 Goose Up/Down migration。
- [ ] Up migration 对现有数据可执行并有明确回填规则。
- [ ] 新索引考虑 `user_id + status + time` 的列表查询。
- [ ] 不修改既有 migration 文件，使用新编号增量迁移。
- [ ] Migration、sqlc 查询和应用逻辑在同一变更中提交。
...
- [ ] 解析失败、重新解析和重复重试。
- [ ] 题集完整替换的事务回滚。
- [ ] 面试跳题、返回改答、重复完成和完成后锁定。
- [ ] 面试刷新后的时间字段一致。
- [ ] 报告首次生成、重复复用和并发复用。
- [ ] Refresh Token 轮换、重放和退出撤销。

### 16.2 契约测试

- [ ] HTTP 状态、响应字段和枚举与 `.api` 一致。
- [ ] ErrorResponse 包含稳定 `code` 和请求 ID。
- [ ] 所有时间为合法 UTC RFC 3339。
- [ ] 所有空集合返回 `[]`。
- [ ] SDK 可直接完成前端核心流程，不需要手写补充 DTO。

### 16.3 性能与可靠性

- [ ] 列表和仪表盘查询无 N+1。
- [ ] 默认分页下核心列表查询达到项目约定延迟目标。
- [ ] 删除、完成面试、重试任务等动作幂等或返回明确冲突。
- [ ] Worker 状态最终落库，失败不会永久停留在 `running`。
- [ ] 日志包含 request ID、用户 ID、资源 ID和错误码，但不包含密码、Token 和完整简历正文。

## 17. 最终后端验收清单

- [ ] 前端所需 P0/P1 接口均写入 `.api`。
- [ ] Go、OpenAPI 和 TypeScript SDK 已重新生成。
- [ ] “简历 → JD → 题集 → 面试 → 报告”完整流程可通过 API 自动化测试。
- [ ] 面试支持跳过、返回覆盖、主动完成和刷新计时恢复。
- [ ] 完成后的面试无法再修改。
- [ ] 简历、JD 和题集管理具备明确关联规则。
- [ ] 仪表盘和任务中心只返回当前用户真实数据。
- [ ] 认证支持短期 Access Token、Refresh Cookie 和退出撤销。
- [ ] 统一分页、错误、时间和状态约定全部落地。
- [ ] 所有新增 migration 可在现有数据上成功执行。
- [ ] 后端测试、生成检查和项目验收脚本全部通过。
