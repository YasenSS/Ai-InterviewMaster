...
func EnsureBucket(ctx context.Context, client *minio.Client, bucket, region string) error {
	...
	if err != nil { return fmt.Errorf("check object bucket: %w", err) }
	if exists { return nil }
	if err := client.MakeBucket(ctx, bucket, minio.MakeBucketOptions{Region: region}); err != nil { return fmt.Errorf("create object bucket: %w", err) }
	return nil
}

func endpoint(raw string) (string, bool, error) {
	if !strings.Contains(raw, "://") { return raw, false, nil }
	parsed, err := url.Parse(raw)
	if err != nil || parsed.Host == "" { return "", false, fmt.Errorf("invalid object storage endpoint") }
