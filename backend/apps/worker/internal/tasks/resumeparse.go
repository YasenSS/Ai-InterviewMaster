...
func ResumeParseHandler(db *pgxpool.Pool, store *minio.Client, bucket, tikaURL string) asynq.HandlerFunc {
	return func(ctx context.Context, task *asynq.Task) error {
		...
		if err := db.QueryRow(ctx, `SELECT object_key, content_type FROM resume_versions WHERE id=$1`, payload.VersionID).Scan(&key, &contentType); err != nil {
			return fail(ctx, db, payload, err)
		}
		...
		if text == "" {
			return fail(ctx, db, payload, fmt.Errorf("Tika did not extract readable resume text"))
		}
		tx, err := db.Begin(ctx)
		if err != nil {
			return err
		...
		_, err = tx.Exec(ctx, `UPDATE resume_versions SET extracted_text=$2,processed_at=now(),parse_error=NULL WHERE id=$1`, payload.VersionID, text)
		if err != nil {
			return fail(ctx, db, payload, err)
		}
		_, err = tx.Exec(ctx, `DELETE FROM resume_facts WHERE resume_version_id=$1`, payload.VersionID)
		if err != nil {
			return fail(ctx, db, payload, err)
		}
		_, err = tx.Exec(ctx, `DELETE FROM resume_chunks WHERE resume_version_id=$1`, payload.VersionID)
		if err != nil {
			return fail(ctx, db, payload, err)
		}
		facts, _ := json.Marshal(map[string]string{"summary": firstLine(text)})
		_, err = tx.Exec(ctx, `INSERT INTO resume_facts(resume_version_id,fact_type,fact_key,fact_value,source_excerpt) VALUES($1,'summary','profile',$2,$3)`, payload.VersionID, facts, firstLine(text))
		if err != nil {
			return fail(ctx, db, payload, err)
		}
		for i, chunk := range chunks(text, 1000) {
			_, err = tx.Exec(ctx, `INSERT INTO resume_chunks(resume_version_id,chunk_no,content,token_count,embedding,embedding_model) VALUES($1,$2,$3,$4,$5::vector,$6)`, payload.VersionID, i+1, chunk, len(strings.Fields(chunk)), baselineEmbedding(chunk), "baseline-hash-1536")
			if err != nil {
				return fail(ctx, db, payload, err)
			}
		}
		_, err = tx.Exec(ctx, `UPDATE resumes SET status='completed',updated_at=now() WHERE id=$1`, payload.ResumeID)
		if err != nil {
			return fail(ctx, db, payload, err)
		}
		_, err = tx.Exec(ctx, `
...
func fail(ctx context.Context, db *pgxpool.Pool, p sharedtasks.ResumeParsePayload, err error) error {
	_, _ = db.Exec(ctx, `
		UPDATE async_tasks
		SET status='failed',
		    error_code='RESUME_PARSE_FAILED',
		    error_summary='简历解析失败，请检查文件后重试',
		    error_message=$2,
		    completed_at=now(),
		    updated_at=now()
		WHERE id=$1`, p.TaskID, err.Error())
	_, _ = db.Exec(ctx, `
		UPDATE resumes SET status='failed',updated_at=now() WHERE id=$1`, p.ResumeID)
	_, _ = db.Exec(ctx, `
		UPDATE resume_versions
		SET parse_error='简历解析失败，请检查文件后重试'
...
func baselineEmbedding(text string) string {
	...
	for i := range values {
		values[i] = fmt.Sprintf("%.6f", (float64(seed[i%len(seed)])/127.5)-1)
	}
