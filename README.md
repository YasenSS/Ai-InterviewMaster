...
面向个人求职者的 AI 面试训练平台。仓库包含工程基线、简历与 JD、文字模拟面试、复盘报告以及 Beta 适配接口。

当前 M0～M3 与 Beta 本地能力均已完成，并通过数据库迁移、真实 HTTP 闭环和 Whisper 音频转写验收。

首发 MVP 聚焦文字模拟面试；面经检索与 ASR 位于 `/api/v1/beta`。面经支持本地画像降级，ASR 默认通过 Beta Compose Profile 使用本地 Whisper。
...
完整设计见 [技术方案_v1.md](技术方案_v1.md)，Milestone 0 范围见 [项目开发任务书_MVP.md](项目开发任务书_MVP.md)。
